...
class CheckCeaseIncomeSourceDetailsControllerISpec extends ControllerISpecHelper {
  ...
  def getExpectedFormAction(mtdUserRole: MTDUserRole, incomeSourceType: IncomeSourceType) = {
    if (mtdUserRole == MTDIndividual) {
      routes.CeaseCheckIncomeSourceDetailsController.submit(incomeSourceType).url
    } else {
      routes.CeaseCheckIncomeSourceDetailsController.submitAgent(incomeSourceType).url
    }
  }

  def getExpectedRedirectUri(mtdUserRole: MTDUserRole, incomeSourceType: IncomeSourceType) = {
    if (mtdUserRole == MTDIndividual) {
      routes.IncomeSourceCeasedObligationsController.show(incomeSourceType).url
    } else {
      routes.IncomeSourceCeasedObligationsController.showAgent(incomeSourceType).url
    }
  }
