...
import common.models.incomeSourceDetails.CeaseIncomeSourceData.{dateCeasedField, incomeSourceIdField}
import common.models.incomeSourceDetails.CeaseIncomeSourceData
            ...
            "is authenticated, with a valid enrolment" should {
              s"redirect to Check income source details" when {
                "form is filled correctly" in {
                  ...
                  val expectedUrl = if (mtdUserRole == MTDIndividual) {
                    routes.CeaseCheckIncomeSourceDetailsController.show(incomeSourceType).url
                  } else {
                    routes.CeaseCheckIncomeSourceDetailsController.showAgent(incomeSourceType).url
                  }
