...
import common.helpers.serviceMocks.IncomeTaxViewChangeStub
import common.helpers.serviceMocks.BusinessDetailsStub
import common.models.incomeSourceDetails.{CeaseIncomeSourceData, IncomeSourceDetailsModel}
...
class IncomeSourceCeasedObligationsControllerISpec extends ControllerISpecHelper {
  ...
  val businessCeasedObligationsShowUrl: String = routes.IncomeSourceCeasedObligationsController.show(SelfEmployment).url
  val foreignPropertyCeasedObligationsShowUrl: String = routes.IncomeSourceCeasedObligationsController.show(ForeignProperty).url
  val ukPropertyCeasedObligationsShowUrl: String = routes.IncomeSourceCeasedObligationsController.show(UkProperty).url
        ...
        s"a user is a $mtdUserRole" that {
          "is authenticated, with a valid enrolment" should {
            "render the Business Ceased obligations page with remaining business content when only one business in latency exists and RF FS is turned on" in {
              ...
              BusinessDetailsStub.stubGetIncomeSourceDetailsResponse(testMtditid)(OK, getIncomeSourceResponse(incomeSourceType))
            ...
            "render the Business Ceased obligations page with remaining business content when  all business ceased and RF FS is turned on" in {
              stubAuthorised(mtdUserRole)
              BusinessDetailsStub.stubGetIncomeSourceDetailsResponse(testMtditid)(OK, getIncomeSourceResponse(incomeSourceType, true))
              IncomeTaxViewChangeStub.stubGetNextUpdates(testNino, testObligationsModel)
              setupTestMongoData(incomeSourceType)
