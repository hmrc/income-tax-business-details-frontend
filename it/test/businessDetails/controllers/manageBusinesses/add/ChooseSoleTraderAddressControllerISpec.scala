...
class ChooseSoleTraderAddressControllerISpec extends ControllerISpecHelper {
  ...
  private def getIncomeSourceCheckDetailsControllerUrlByType(isAgent: Boolean) =
    if isAgent then routes.IncomeSourceCheckDetailsController.showAgent(SelfEmployment).url
    else routes.IncomeSourceCheckDetailsController.show(SelfEmployment).url
        ...
        "is authenticated, with a valid enrolment" should {
          ...
          "reload the page" when {
            ...
            "form response is New Address" in {
              ...
              result should have(
                httpStatus(SEE_OTHER),
                redirectURI(routes.IsTheNewAddressInTheUKController.show(isAgent).url)
              )
            }
          }
