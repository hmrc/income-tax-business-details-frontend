        ...
        s"a user is a $mtdUserRole" that {
          "is authenticated, with a valid enrolment" should {
            s"submit the data after the user has checked their details" in {
              ...
              val redirectUri = if (isAgent) routes.IncomeSourceAddedController.showAgent(incomeSourceType).url else routes.IncomeSourceAddedController.show(incomeSourceType).url
