...
import helpers.servicemocks.CalculationListStub
import common.helpers.serviceMocks.BusinessDetailsStub
import common.models.incomeSourceDetails._
          ...
          "is authenticated, with a valid enrolment" should {
            s"render the ${incomeSourceType.journeyType} Reporting Method page" when {
              ...
              "user is within latency period (before 23/24) - tax year 1 not crystallised" in {
                ...
                BusinessDetailsStub.stubGetIncomeSourceDetailsResponse(testMtditid)(OK, getIncomeSourceDetailsResponse(incomeSourceType, true))
                await(sessionService.setMongoData(testUIJourneySessionData(incomeSourceType)))

                CalculationListStub.stubGetCalculationList(testNino, taxYear1.toString)(CalculationListIntegrationTestConstants.successResponseNonCrystallised.toString())
              ...
              "user is within latency period (before 23/24) - tax year 1 crystallised" in {
                ...
                CalculationListStub.stubGetCalculationList(testNino, taxYear1.toString)(CalculationListIntegrationTestConstants.successResponseCrystallised.toString())
