...
import common.models.incomeSourceDetails.AddIncomeSourceData.businessNameField
...
class AddBusinessNameControllerISpec extends ControllerISpecHelper {
  ...
  def backUrl(mode: Mode, isAgent: Boolean): String = {
    if (mode == CheckMode) {
      val incomeSourceController = routes.IncomeSourceCheckDetailsController
      if(isAgent) incomeSourceController.showAgent(SelfEmployment).url else incomeSourceController.show(SelfEmployment).url
    } else {
      val manageBusinessController = manageBusinessRoutes.ManageYourBusinessesController
          ...
          s"303 SEE_OTHER and redirect to add business start date" when {
            "using the manage businesses journey" in {
              ...
              result should have(
                ...
                redirectURI(routes.AddIncomeSourceStartDateController.show(incomeSourceType = SelfEmployment, isAgent = mtdUserRole != MTDIndividual, mode = NormalMode).url)
              )
      ...
      s"a user is a $mtdUserRole" that {
        "is authenticated, with a valid enrolment" should {
          val expectedRedirectUrl = {
            val controllerRoute = routes.IncomeSourceCheckDetailsController
