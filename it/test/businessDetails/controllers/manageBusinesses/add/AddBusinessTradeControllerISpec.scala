...
import common.models.incomeSourceDetails.AddIncomeSourceData.businessTradeField
        ...
        "is authenticated, with a valid enrolment" should {
          "render the Add Business trade page for an Agent" when {
            "using the manage businesses journey" in {
              ...
              BusinessDetailsStub.stubGetIncomeSourceDetailsResponse(testMtditid)(OK, multipleBusinessesResponse)
        ...
        "is authenticated, with a valid enrolment" should {
          s"303 SEE_OTHER and redirect to add business address" when {
            "User is authorised and business trade is valid" in {
              stubAuthorised(mtdUserRole)
              BusinessDetailsStub.stubGetIncomeSourceDetailsResponse(testMtditid)(OK, noPropertyOrBusinessResponse)
              ...
              val expectedUrl = if(mtdUserRole == MTDIndividual) {
                routes.AddBusinessAddressController.show(mode = NormalMode).url
              } else {
                routes.AddBusinessAddressController.showAgent(mode = NormalMode).url
              }
              result should have(
            ...
            "the OverseasBusinessAddress FS is on" when {
              "there is address on file" when {
                "User is authorised and business trade is valid" in {
                  ...
                  BusinessDetailsStub.stubGetIncomeSourceDetailsResponse(testMtditid)(OK, singleBusinessResponse)
                  ...
                  val expectedUrl = if (mtdUserRole == MTDIndividual) {
                    routes.ChooseSoleTraderAddressController.show(false).url
                  } else {
                    routes.ChooseSoleTraderAddressController.show(true).url
                  }
                  result should have(
                    httpStatus(SEE_OTHER),
                    redirectURI(expectedUrl)
            ...
            "the OverseasBusinessAddress FS is on" when {
              "there is no address on file" when {
                "User is authorised and business trade is valid" in {
                  ...
                  val expectedUrl = if (mtdUserRole == MTDIndividual) {
                    routes.IsTheNewAddressInTheUKController.show(false).url
                  } else {
                    routes.IsTheNewAddressInTheUKController.show(true).url
                  }
                  result should have(
                    httpStatus(SEE_OTHER),
                    redirectURI(expectedUrl)
