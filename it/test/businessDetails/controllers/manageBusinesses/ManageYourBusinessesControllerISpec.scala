...
class ManageYourBusinessesControllerISpec extends ControllerISpecHelper {
  ...
  val showIndividualViewIncomeSourceControllerUrl: String = routes.ManageYourBusinessesController.show().url
        ...
        "is authenticated, with a valid enrolment" should {
          "render the manage your businesses page" when {
            "the user has multiple businesses and uk property with display business start date enabled" in {
              ...
              BusinessDetailsStub.stubGetIncomeSourceDetailsResponse(testMtditid)(OK, multipleBusinessesAndUkProperty)
              ...
              result should have(
                ...
                elementAttributeBySelector("#back-fallback", "href")(appConfig.homePageUrl(mtdUserRole != MTDIndividual)),
              )
            }

            "the user has foreign property and ceased business with display business start date enabled" in {
              stubAuthorised(mtdUserRole, List(DisplayBusinessStartDate))
              BusinessDetailsStub.stubGetIncomeSourceDetailsResponse(testMtditid)(OK, foreignPropertyAndCeasedBusiness)
