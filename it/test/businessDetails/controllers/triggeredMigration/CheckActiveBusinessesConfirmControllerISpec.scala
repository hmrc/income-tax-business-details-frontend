...
import testConstants.IncomeSourceDetailsTestConstants.{singleBusinessIncome, singleBusinessIncomeUnconfirmed, singleBusinessIncomeWithYearOfMigration}
...
class CheckActiveBusinessesConfirmControllerISpec extends ControllerISpecHelper {
  ...
  mtdAllRoles.foreach { mtdRole =>
    ...
    val expectedRedirect: String = appConfig.homePageUrl(mtdRole != MTDIndividual)
    ...
    s"GET $path" when {
      s"user is $mtdRole" should {
        "render the page when TriggeredMigration FS is enabled - With Year of Migration" in {
          ...
          BusinessDetailsStub.stubGetIncomeSourceDetailsResponse(testMtditid)(OK, singleBusinessIncomeWithYearOfMigration)
        ...
        "render the page when TriggeredMigration FS is enabled - With no Year of Migration" in {
          stubAuthorised(mtdRole, List(TriggeredMigration))
          BusinessDetailsStub.stubGetIncomeSourceDetailsResponse(testMtditid)(OK, singleBusinessIncomeUnconfirmed)
          ITSAStatusDetailsStub.stubGetITSAStatusFutureYearsDetails(TaxYear(2023, 2024), ITSAStatus.Voluntary, ITSAStatus.Voluntary, ITSAStatus.Voluntary, "AA123456A")
          IncomeTaxCalculationStub.stubGetCalculationResponse("AA123456A", "2018", Some("LATEST"))(
            status = OK,
            body = liabilityCalculationModelSuccessfulNotCrystallised
        ...
        "redirect to home page when TriggeredMigration FS is disabled" in {
          ...
          BusinessDetailsStub.stubGetIncomeSourceDetailsResponse(testMtditid)(OK, singleBusinessIncome)
    ...
    s"POST $path" when {
      s"user is $mtdRole" should {
        "redirect to the complete page when 'Yes' is selected" in {
          ...
          BusinessDetailsStub.stubUpdateCustomerFacts(testMtditid)(OK)
          ...
          result should have(
            httpStatus(SEE_OTHER),
            redirectURI(s"/manage-self-assessment/businesses$completePath")
          )
        ...
        "redirect to Check HMRC Records when 'No' is selected" in {
          ...
          result should have(
            httpStatus(SEE_OTHER),
            redirectURI(s"/manage-self-assessment/businesses$checkHmrcRecordsPath")
          )
        }
