...
object AddPropertyForm {
  ...
  def apply: Form[AddPropertyForm] = {
    ...
    Form[AddPropertyForm](
      ...
      (response => AddPropertyForm(response))
      (form => Some(form.response))
    )
  }
}

case class AddPropertyForm(response: Option[String]) {
  def toFormMap: Map[String, Seq[String]] = Map(
    AddPropertyForm.response -> Seq(response.getOrElse("N/A"))
  )
}
