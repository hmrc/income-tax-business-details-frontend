...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.audit.models.CreateIncomeSourceAuditModel
...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.enums.BeforeSubmissionPage
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.enums.IncomeSourceJourney.*
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.enums.JourneyType.{Add, IncomeSourceJourneyType}
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.enums.TriggeredMigration.TriggeredMigrationAdded
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.UIJourneySessionData
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.admin.OverseasBusinessAddress
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.core.NormalMode
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.createIncomeSource.CreateIncomeSourceResponse
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.incomeSourceDetails.IncomeSourceDetailsModel
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.incomeSourceDetails.viewmodels.{CheckBusinessDetailsViewModel, CheckDetailsViewModel, CheckPropertyViewModel}
import play.api.Logger
import play.api.i18n.I18nSupport
import play.api.mvc.*
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.config.featureswitch.FeatureSwitching
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.services.{CreateBusinessDetailsService, SessionService}
import uk.gov.hmrc.play.bootstrap.frontend.controller.FrontendController
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.utils.JourneyCheckerManageBusinesses
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.views.html.manageBusinesses.add.IncomeSourceCheckDetailsView
...
class IncomeSourceCheckDetailsController @Inject()(val incomeSourceCheckDetailsView: IncomeSourceCheckDetailsView,
  ...
  with JourneyCheckerManageBusinesses with FeatureSwitching with I18nSupport {
      ...
      viewModel match {
        case Some(viewModel) =>
          businessDetailsService.createRequest(viewModel) flatMap {
            ...
            case Left(ex) =>
              auditingService.extendedAudit(
                CreateIncomeSourceAuditModel(incomeSourceType, viewModel, Some(uk.gov.hmrc.incometaxbusinessdetailsfrontend.enums.FailureCategory.ApiFailure), Some(ex.getMessage), None, isTrigMig = isTriggeredMigration)
              )
              Future.failed(ex)
          }
        case None =>
