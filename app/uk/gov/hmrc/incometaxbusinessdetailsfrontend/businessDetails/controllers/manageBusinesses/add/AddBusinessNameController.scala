...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.enums.IncomeSourceJourney.SelfEmployment
...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.forms.manageBusinesses.add.BusinessNameForm
...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.utils.{IncomeSourcesUtils, JourneyCheckerManageBusinesses}
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.views.html.manageBusinesses.add.AddBusinessNameView
...
class AddBusinessNameController @Inject()(val authActions: AuthActions,
  ...
  private def getBackUrl(isAgent: Boolean, mode: Mode): String = {
    ((isAgent, mode) match {
      case (false, NormalMode) => manageBusinessesRoutes.ManageYourBusinessesController.show()
      case (_, NormalMode) => manageBusinessesRoutes.ManageYourBusinessesController.showAgent()
