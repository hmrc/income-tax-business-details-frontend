...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.enums.JourneyType.{Opt, SignUpJourney}
...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.obligations.models.reportingObligations.signUp.SignUpSessionData
...
class SignUpSessionDataRepository @Inject()(val repository: UIJourneySessionDataRepository) {
  ...
  def setupSessionData()(implicit hc: HeaderCarrier): Future[Boolean] =
    repository.set(createDefaultUIJourneySessionData())
  ...
  def createDefaultUIJourneySessionData()(implicit hc: HeaderCarrier): UIJourneySessionData =
    UIJourneySessionData(hc.sessionId.get.value, Opt(SignUpJourney).toString, signUpSessionData = Some(SignUpSessionData(None, None, Some(false))))
}