...
package uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.creditsandrefunds
...
case class PaymentCreditRow(transactionId: String, amount: BigDecimal, date: LocalDate, effectiveDate: LocalDate) extends CreditRow {
  ...
  def descriptionLink(isAgent: Boolean): String =
    if (isAgent) routes.PaymentAllocationsController.viewPaymentAllocationAgent(transactionId).url
    else routes.PaymentAllocationsController.viewPaymentAllocation(transactionId).url
}
