...
package uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.liabilitycalculation.viewmodels

import uk.gov.hmrc.incometaxbusinessdetailsfrontend.auth.MtdItUser
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.financialDetails.SecondLatePaymentPenalty
...
case class TaxYearSummaryViewModel(
  ...
  def getForecastSummaryHref(taxYear: Int, isAgent: Boolean): String = {
    if (isAgent) {
      uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.ForecastIncomeSummaryController.showAgent(taxYear).url
    } else {
      uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.ForecastIncomeSummaryController.show(taxYear).url
    }
  }

  def getForecastTaxDueHref(taxYear: Int, isAgent: Boolean): String = {
    if (isAgent) {
      uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.ForecastTaxCalcSummaryController.showAgent(taxYear).url
    } else {
      uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.ForecastTaxCalcSummaryController.show(taxYear).url
    }
  }
  ...
  def getChargeSummaryHref(chargeItem: TaxYearSummaryChargeItem,
    ...
    if (chargeItem.transactionType == SecondLatePaymentPenalty) {
      ...
      if (isAgent) {
        uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.ChargeSummaryController.showAgent(taxYear, chargeItem.transactionId, chargeItem.isAccruingInterest).url
      } else {
        uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.ChargeSummaryController.show(taxYear, chargeItem.transactionId, chargeItem.isAccruingInterest, origin).url
      }
    }
  }
}
