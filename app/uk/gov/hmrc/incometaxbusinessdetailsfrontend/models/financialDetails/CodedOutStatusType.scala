...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.enums.CodingOutType._
...
object CodedOutStatusType {
  ...
  def fromCodedOutStatusAndDocumentText(documentText: Option[String], codedOutStatus: Option[String]): Option[CodedOutStatusType] = {
    (documentText, codedOutStatus) match {
      ...
      case (_, Some(CODING_OUT_ACCEPTED.code))          => Some(Accepted)
      case (_, Some(CODING_OUT_CANCELLED.code))         => Some(Cancelled)
      case (_, Some(CODING_OUT_FULLY_COLLECTED.code))   => Some(FullyCollected)
      case (_, Some(CODING_OUT_NOT_COLLECTED.code))     => Some(Cancelled)
      case (_, Some(CODING_OUT_PARTLY_COLLECTED.code))  => Some(Cancelled)
      case _ => None
    }
  }
