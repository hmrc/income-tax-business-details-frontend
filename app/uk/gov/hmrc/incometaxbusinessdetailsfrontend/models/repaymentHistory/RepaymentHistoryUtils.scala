...
package uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.repaymentHistory

import uk.gov.hmrc.incometaxbusinessdetailsfrontend.exceptions.MissingFieldException
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.implicits.ImplicitCurrencyFormatter.CurrencyFormatter
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.financialDetails._
import play.api.Logger
import play.api.i18n.Messages
import play.api.libs.json.Json
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.services.DateServiceInterface
...
object RepaymentHistoryUtils {
  ...
  private def getControllerHref(transactionId: Option[String], isAgent: Boolean) = {
    if (isAgent) {
      uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.PaymentAllocationsController.viewPaymentAllocationAgent(transactionId.getOrElse(throw MissingFieldException("Transaction ID"))).url
    } else {
      uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.PaymentAllocationsController.viewPaymentAllocation(transactionId.getOrElse(throw MissingFieldException("Transaction ID"))).url
    }
  }
  ...
  private def getCreditsLinkUrl(date: LocalDate, isAgent: Boolean) = {
    ...
    if (isAgent) {
      uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.CreditsSummaryController.showAgentCreditsSummary(year).url
    } else {
      uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.CreditsSummaryController.showCreditsSummary(year).url
    }
  }
  ...
  def getChargeLinkUrl(isAgent: Boolean, taxYear: Int, chargeId: String, codedOut: Option[Boolean] = None): String = {
    val baseUrl =
      if (isAgent) {
        uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.ChargeSummaryController.showAgent(taxYear, chargeId).url
      } else {
        uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.ChargeSummaryController.show(taxYear, chargeId).url
      }
  ...
  private def codedOutChargeEntry(chargeItem: ChargeItem, isAgent: Boolean)(implicit dateServiceInterface: DateServiceInterface): PaymentHistoryEntry = {
    PaymentHistoryEntry(
      date = chargeItem.creationDate.getOrElse(chargeItem.documentDate),
