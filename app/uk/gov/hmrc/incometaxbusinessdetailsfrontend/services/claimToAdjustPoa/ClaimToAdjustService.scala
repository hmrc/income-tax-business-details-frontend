...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.connectors.{CalculationListConnector, ChargeHistoryConnector, FinancialDetailsConnector}
...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.financialDetails.{FinancialDetailsErrorModel, FinancialDetailsModel}
    ...
    checkCrystallisation(nino, getPoaAdjustableTaxYears)(hc, dateService, calculationListConnector, ec).flatMap {
      ...
      case Some(taxYear: TaxYear) =>
        financialDetailsConnector.getFinancialDetails(taxYear.endYear, nino.value).map {
          ...
          case _ =>
            Right(FinancialDetailsAndPoaModel(None, Some(generateZeroedPoas(taxYear))))
        }
    }
  }
