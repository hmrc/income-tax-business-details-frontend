...
package uk.gov.hmrc.incometaxbusinessdetailsfrontend.utils.claimToAdjust

import uk.gov.hmrc.incometaxbusinessdetailsfrontend.auth.MtdItUser
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.enums.{AfterSubmissionPage, BeforeSubmissionPage, CannotGoBackPage, InitialPage, JourneyState}
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.claimToAdjustPoa.PoaAmendmentData
...
trait JourneyCheckerClaimToAdjust extends ErrorRecovery {
  ...
  def redirectToYouCannotGoBackPage(user: MtdItUser[_]): Result = {
    Redirect(uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.claimToAdjustPoa.routes.YouCannotGoBackController.show(user.isAgent).url)
  }
