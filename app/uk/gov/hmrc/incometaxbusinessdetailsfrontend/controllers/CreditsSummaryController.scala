...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.audit.models.CreditSummaryAuditing.{CreditsSummaryModel,toCreditSummaryDetailsSeq}
...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.creditDetailModel.CreditDetailModel
import play.api.Logger
import play.api.i18n.{I18nSupport, MessagesApi}
import play.api.mvc.{Action, AnyContent, MessagesControllerComponents, Result}
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.services.CreditHistoryService
import uk.gov.hmrc.http.HeaderCarrier
import uk.gov.hmrc.play.bootstrap.frontend.controller.FrontendController
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.views.html.CreditsSummaryView
...
class CreditsSummaryController @Inject()(
                                          creditsView: CreditsSummaryView,
  ...
  private def creditsSummaryUrl(calendarYear: Int, origin: Option[String]): String =
    uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.CreditsSummaryController.showCreditsSummary(calendarYear, origin).url

  lazy val moneyInYourAccountUrl: String = uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.MoneyInYourAccountController.show().url

  private def paymentHistoryUrl(origin: Option[String]): String =
    uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.PaymentHistoryController.show(origin).url

  private def agentCreditsSummaryUrl(calendarYear: Int): String =
    uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.CreditsSummaryController.showAgentCreditsSummary(calendarYear).url

  private lazy val agentMoneyInYourAccountUrl: String = uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.MoneyInYourAccountController.showAgent().url

  private lazy val agentPaymentHistoryHomeUrl: String = uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.PaymentHistoryController.showAgent().url
