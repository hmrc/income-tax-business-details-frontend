...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.audit.models.ChargeSummaryAudit
...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.ChargeSummaryController.ErrorCode
...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.admin.*
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.chargeHistory.*
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.chargeSummary.{ChargeSummaryViewModel, PaymentHistoryAllocations}
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.core.Nino
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.financialDetails.*
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.incomeSourceDetails.TaxYear
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.paymentAllocationCharges.PaymentAllocationViewModel
import play.api.Logger
import play.api.i18n.I18nSupport
import play.api.mvc.*
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.services.{ChargeHistoryService, DateServiceInterface, FinancialDetailsService, PaymentAllocationsService}
import uk.gov.hmrc.http.HeaderCarrier
import uk.gov.hmrc.play.bootstrap.frontend.controller.FrontendController
import uk.gov.hmrc.play.language.LanguageUtils
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.utils.FallBackBackLinks
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.views.html.{ChargeSummaryView, YourSelfAssessmentChargeSummaryView}
...
class ChargeSummaryController @Inject()(val authActions: AuthActions,
  ...
  def onError(message: String, isAgent: Boolean, showInternalServerError: Boolean)(implicit request: Request[_]): Result = {
    ...
    if (showInternalServerError) {
      ...
      if (isAgent) Redirect(uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.agent.errors.routes.AgentNotFoundDocumentIDLookupController.show().url)
      else Redirect(uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.errors.routes.NotFoundDocumentIDLookupController.show().url)
    }
  }
  ...
  def handleRequest(taxYear: Int, id: String, isInterestCharge: Boolean = false, isAgent: Boolean, origin: Option[String] = None)
    ...
    financialDetailsService.getAllFinancialDetails.flatMap { financialResponses =>
      ...
      val codedOutParam: Option[Boolean] = user.getQueryString("codedOut").flatMap {
        case "true" => Some(true)
      ...
      if (matchingModels.nonEmpty) {

        val preferredByRequestedYear: Option[(Int, FinancialDetailsModel)] = matchingModels.find(_._1 == taxYear)

        val preferredModel: Option[(Int, FinancialDetailsModel)] = codedOutParam match {
          case Some(true) =>
            matchingModels.find { case (_, fdm) =>
              fdm.asChargeItems.find(_.transactionId == id).exists(_.isCodingOut)
            }.orElse(preferredByRequestedYear).orElse(matchingModels.headOption)

          case Some(false) =>
            matchingModels.find { case (_, fdm) =>
              fdm.asChargeItems.find(_.transactionId == id).exists(ci => !ci.isCodingOut)
            }.orElse(preferredByRequestedYear).orElse(matchingModels.headOption)

          case None =>
            preferredByRequestedYear.orElse(matchingModels.headOption)
        }

        preferredModel match {
          case Some((selectedTaxYear, fdmForTaxYear)) =>
            if (selectedTaxYear != taxYear) {

              val redirectCall =
                if (isAgent) routes.ChargeSummaryController.showAgent(selectedTaxYear, id, isInterestCharge)
                else routes.ChargeSummaryController.show(selectedTaxYear, id, isInterestCharge, origin)

              val redirectUrl =
                codedOutParam match {
                  case Some(value) => s"${redirectCall.url}?codedOut=${value.toString}"
                  case None => redirectCall.url
                }

              Future.successful(Redirect(redirectUrl))
            } else {
              doShowChargeSummary(selectedTaxYear, id, isInterestCharge, fdmForTaxYear, paymentsFromAllYears, isAgent, origin, isMFADebit(fdmForTaxYear, id))
            }
  ...
  private def doShowChargeSummary(taxYear: Int, id: String, isInterestCharge: Boolean,
    ...
    chargeDetailsforTaxYear.findDocumentDetailByIdWithDueDate(id) match {
      case Some(documentDetailWithDueDate) =>
        ...
        if (chargeItem.isPenalty && !isEnabled(PenaltiesAndAppeals)) {
          ...
          chargeHistoryService.chargeHistoryResponse(isInterestCharge, chargeReference, isEnabled(ChargeHistory)).flatMap {
            case Right(chargeHistory) =>
              ...
              val whatYouOweUrl = {
                if (isAgent) uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.WhatYouOweController.showAgent().url
                else uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.WhatYouOweController.show(origin).url
              }
  ...
  private def checkForPoaExtraChargeLink(
    ...
    chargeItem match {
      case Some(value) =>
        ...
        extraChargeId match {
          case Some(validId) =>
            if (isAgent) Some(uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.ChargeSummaryController.showAgent(documentDetailWithDueDate.documentDetail.taxYear, validId).url)
            else Some(uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.ChargeSummaryController.show(documentDetailWithDueDate.documentDetail.taxYear, validId).url)
          case None => None
        }
      case None => None
    }
