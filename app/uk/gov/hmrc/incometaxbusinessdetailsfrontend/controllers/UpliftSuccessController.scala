...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.audit.models.IvOutcomeSuccessAuditModel
...
class UpliftSuccessController @Inject()(authActions: AuthActions,
  ...
  def success(): Action[AnyContent] = authActions.asMTDIndividual().async {
    implicit user =>
      ...
      Future.successful(Redirect(uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.HomeController.show().url))
  }
}
