...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.views.html.agent.errorPages.UTRErrorView
...
class UTRErrorController @Inject()(utrError: UTRErrorView,
  ...
  def show: Action[AnyContent] = authActions.asAgent().async { implicit user =>
    ...
    Future.successful(Ok(utrError(
      postAction = uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.agent.routes.UTRErrorController.submit()
    )))
  }
