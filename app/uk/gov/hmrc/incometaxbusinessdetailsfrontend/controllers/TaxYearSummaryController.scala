...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.audit.models.TaxYearSummaryResponseAuditModel
...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.enums.GatewayPage.TaxYearSummaryPage
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.forms.utils.SessionKeys.{calcPagesBackPage, gatewayPage}
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.implicits.ImplicitDateFormatter
...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.liabilitycalculation.*
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.liabilitycalculation.viewmodels.{CalculationSummary, TYSClaimToAdjustViewModel, TaxYearSummaryViewModel}
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.taxyearsummary.TaxYearSummaryChargeItem
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.obligations.models.ObligationsModel
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.obligations.services.NextUpdatesService
import play.api.Logger
import play.api.i18n.{I18nSupport, Lang, Messages, MessagesApi}
import play.api.mvc.*
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.services.*
...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.views.html.TaxYearSummaryView
...
class TaxYearSummaryController @Inject()(
  ...
  private def taxYearsUrl(origin: Option[String]): String = uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.TaxYearsController.showTaxYears(origin).url

  private def whatYouOweUrl(origin: Option[String]): String = uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.WhatYouOweController.show(origin).url

  private def homeUrl(origin: Option[String]): String = uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.HomeController.show(origin).url

  // Agent back urls
  private lazy val agentTaxYearsUrl: String = uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.TaxYearsController.showAgentTaxYears().url
  private lazy val agentHomeUrl: String = uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.HomeController.showAgent().url
  private lazy val agentWhatYouOweUrl: String = uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.WhatYouOweController.showAgent().url
    ...
    liabilityCalc.messages match {
      case Some(value) =>
        ...
        val translatedDateMessages =
          uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.liabilitycalculation.Messages.translateMessageDateVariables(errorMessages)(messages, this)
  ...
  private def handleCalcSuccess(
    ...
    lazy val ctaLink = uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.claimToAdjustPoa.routes.AmendablePoaController.show(isAgent = isAgent).url
      ...
      case financialDetails@FinancialDetailsModel(_, _, documentDetails, fd) =>
        ...
        val chargeItemsCodingOutPaye: List[TaxYearSummaryChargeItem] = {
          chargeItemsCodingOut
            .filter(_.codedOutStatus.contains(uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.financialDetails.Accepted))
            .filterNot(_.originalAmount <= 0)
        }
        ...
        val chargeItemsCodingOutNotPaye: List[TaxYearSummaryChargeItem] = {
          chargeItemsCodingOut
            .filterNot(_.codedOutStatus.contains(uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.financialDetails.Accepted))
            .filterNot(_.originalAmount <= 0)
        }
