...
package uk.gov.hmrc.incometaxbusinessdetailsfrontend.auth.authV2.actions
...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.connectors.IncomeTaxCalculationConnector
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.BaseController
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.enums.JourneyType.TriggeredMigrationJourney
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.enums.TaxYearSummary.CalculationRecord.LATEST
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.admin.TriggeredMigration
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.liabilitycalculation.{LiabilityCalculationError, LiabilityCalculationResponse}
import play.api.Logger
import play.api.mvc.{ActionRefiner, MessagesControllerComponents, Result}
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.services.{CustomerFactsUpdateService, DateServiceInterface, ITSAStatusService, SessionService}
...
class TriggeredMigrationRetrievalAction @Inject()(
  ...
  private def isItsaStatusVoluntaryOrMandated()(implicit hc: HeaderCarrier, user: MtdItUser[_]): Future[Either[Result, Boolean]] = {
    def redirectBasedOnUser: Future[Either[Result, Boolean]] =
      Future(Left(if (user.isAgent) Redirect(uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.HomeController.showAgent()) else Redirect(uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.HomeController.show())))
  ...
  private def redirectToHome(isAgent: Boolean): Result = {
    if (isAgent) {
      Redirect(uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.HomeController.showAgent())
    } else {
      Redirect(uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.HomeController.show())
    }
  }
