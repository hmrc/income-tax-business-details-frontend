...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.auth.authV2.AuthExceptions.NoAssignment
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.auth.authV2.models.{AuthorisedAgentWithClientDetailsRequest, AuthorisedAndEnrolledRequest}
...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.enums.{MTDPrimaryAgent, MTDSupportingAgent, MTDUserRole}
  ...
  def handleAuthFailure[A](throwable: Throwable)(implicit request: Request[_]): Future[Either[Result, AuthorisedAndEnrolledRequest[A]]] = {
    throwable match {
      case _: BearerTokenExpired =>
        ...
        Future.successful(Left(Redirect(uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.timeout.routes.SessionTimeoutController.timeout())))
      case _: InsufficientEnrolments =>
        logger.error(s"missing delegated enrolment. Redirect to agent error page.")
        Future.successful(Left(Redirect(uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.agent.routes.ClientRelationshipFailureController.show())))
      case _: NoAssignment =>
        logger.error(s"Agent User is not in an access group associated with the Client.")
        Future.successful(Left(Redirect(uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.agent.routes.NoAssignmentController.show())))
      case authorisationException: AuthorisationException =>
        ...
        Future.successful(Left(Redirect(uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.SignInController.signIn())))
      case ex =>
