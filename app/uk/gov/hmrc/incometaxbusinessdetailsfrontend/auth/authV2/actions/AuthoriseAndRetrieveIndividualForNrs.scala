...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.audit.models.IvUpliftRequiredAuditModel
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.auth.*
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.auth.authV2.Constants
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.auth.authV2.models.{AuthUserDetails, AuthorisedAndEnrolledRequest}
import com.google.inject.Singleton
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.config.FrontendAppConfig
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.agent.AuthUtils.mtdEnrolmentName
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.enums.MTDIndividual
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.forms.utils.SessionKeys
...
class AuthoriseAndRetrieveIndividualForNrs @Inject()(val authorisedFunctions: FrontendAuthorisedFunctions,
  ...
  def ivUpliftRedirectUrl[A](implicit request: Request[A]):String = {
    ...
    val completionUrl: String = s"$host${uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.UpliftSuccessController.success().url}"
    val failureUrl: String = s"$host${uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.errors.routes.UpliftFailedController.show().url}"
  ...
  private def redirectIfAgentNrs[A]()(
    ...
    case _ ~ _ ~ _ ~ Some(Agent) ~ _ ~ _ ~ _ ~ _ ~ _ ~ _ ~ _ ~ _ ~ _ ~ _ ~ _ ~ _ ~ _ ~ _ =>
      ...
      Future.successful(Left(Redirect(uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.agent.routes.EnterClientsUTRController.show())))
  }
