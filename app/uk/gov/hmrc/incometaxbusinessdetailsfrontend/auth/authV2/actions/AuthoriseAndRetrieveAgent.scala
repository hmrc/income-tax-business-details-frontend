...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.auth.authV2.models.{AuthUserDetails, AuthorisedUserRequest}
...
class AuthoriseAndRetrieveAgent @Inject()(val authorisedFunctions: FrontendAuthorisedFunctions,
  ...
  def logAndRedirect[A]: PartialFunction[Throwable, Future[Either[Result, AuthorisedUserRequest[A]]]] = {
    ...
    case _: InsufficientEnrolments =>
      ...
      Future.successful(Left(Redirect(uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.agent.errors.routes.AgentErrorController.show())))
  ...
  private def redirectIfNotAgent[A]()(
    ...
    case _ ~ _ ~ Some(ag@(Organisation | Individual)) ~ _ =>
      ...
      Future.successful(Left(Redirect(uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.routes.HomeController.show())))
  }
}
