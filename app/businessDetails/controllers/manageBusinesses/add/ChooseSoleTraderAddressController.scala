...
import common.models.incomeSourceDetails.{AddIncomeSourceData, Address, ChooseSoleTraderAddressUserAnswer, Country}
...
class ChooseSoleTraderAddressController @Inject()(
  ...
  private def backUrl(isAgent: Boolean): String =
    if (isAgent) appConfig.homePageUrl(true)
    else appConfig.homePageUrl(false)
  ...
  private def buildAddressOptions(user: MtdItUser[_]): Seq[(String, Int)] = {
    user.incomeSources.getAllUniqueBusinessAddressesWithIndex.map {
      case (answer, index) =>
        val id = index
        val display = {
          (answer.addressLine1, answer.postcode) match {
            case (Some(addressLine1), Some(postcode)) =>
              s"$addressLine1, $postcode"
            case (Some(addressLine1), None) =>
              s"$addressLine1"
            case _ => ""
          }
        }
          ...
          case None =>
            formResponse match {
              case "new-address" =>
                ...
                val redirect = Redirect(routes.IsTheNewAddressInTheUKController.show(isAgent))
  ...
  def show(isAgent: Boolean, isTriggeredMigration: Boolean): Action[AnyContent] = authActions.asMTDIndividualOrAgentWithClient(isAgent = isAgent, triggeredMigrationPage = isTriggeredMigration).async { implicit user =>
    if (isEnabled(OverseasBusinessAddress)) {
      ...
      val homeCall = if (isAgent) Call("GET", appConfig.homePageUrl(true)) else Call("GET", appConfig.homePageUrl(false))
