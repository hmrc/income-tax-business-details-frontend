...
import common.models.obligations.audit.NextUpdatesResponseAuditModel
...
class ObligationsConnector @Inject()(val http: HttpClientV2,
  ...
  def getOpenObligationsUrl(nino: String): String = {
    s"${appConfig.incomeTaxObligationsBaseUrl}/income-tax-obligations/$nino/open-obligations"
  }

  def getAllObligationsDateRangeUrl(fromDate: LocalDate, toDate: LocalDate, nino: String): String = {
    s"${appConfig.incomeTaxObligationsBaseUrl}/income-tax-obligations/$nino/obligations/from/$fromDate/to/$toDate"
  }

  def getFulfilledObligationsUrl(nino: String): String = {
    s"${appConfig.incomeTaxObligationsBaseUrl}/income-tax-obligations/$nino/fulfilled-obligations"
  }
