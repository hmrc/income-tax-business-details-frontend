...
import controllers.manageBusinesses.routes as manageBusinessesRoutes
...
import auth.{AuthActions, MtdItUser}
...
class ManageObligationsController @Inject()(val authActions: AuthActions,
          ...
          (incomeSourceType, taxYearOpt, reportingMethodOpt, incomeSourceIdStringOpt) match {
            case (SelfEmployment, Some(taxYear), Some(reportingMethod), incomeSourceIdStringOpt) =>
              ...
              handleRequest(
                ...
                isAgent = isAgent,
                taxYear = s"${taxYear - 1}-$taxYear",
                changeTo = reportingMethod,
  ...
  private lazy val successPostUrl = (isAgent: Boolean) =>
    routes.ManageObligationsController.submit(isAgent)
      ...
      case (Some(years), AnnualReportingMethod.name | QuarterlyReportingMethod.name) =>
        getIncomeSourceId(incomeSourceType, incomeSourceId, isAgent = isAgent) match {
          case Some(incomeSourceId) =>
            ...
            nextUpdatesService
              .getObligationsViewModel(incomeSourceId.value, showPreviousTaxYears = false)
              .map { viewModel =>
                Ok(obligationsView(
                  sources = viewModel,
          ...
          case None =>
            showError(isAgent, s"Unable to retrieve income source ID for $incomeSourceType") // 500
        }
      case (Some(_), _) =>
        showError(isAgent, s"Invalid changeTo mode provided: -$changeTo-") // 500
      case (None, _) =>
        showError(isAgent, "Invalid tax year provided") // 500
    }
  }
  ...
  def submit(isAgent: Boolean): Action[AnyContent] =
    authActions.asMTDIndividualOrAgentWithClient(isAgent).async { _ =>
      if (isAgent) Future(Redirect(manageBusinessesRoutes.ManageYourBusinessesController.showAgent()))
      else Future(Redirect(manageBusinessesRoutes.ManageYourBusinessesController.show()))
    }
}
