...
import models.audit.ManageIncomeSourceCheckYourAnswersAuditModel
...
class CheckYourAnswersController @Inject()(val checkYourAnswers: CheckYourAnswersView,
  ...
  def show(isAgent: Boolean,
    ...
    authActions.asMTDIndividualOrAgentWithClient(isAgent).async { implicit user =>
      withSessionData(IncomeSourceJourneyType(Manage, incomeSourceType), journeyState = BeforeSubmissionPage) { sessionData =>
        ...
        val changeToStringOpt = sessionData.manageIncomeSourceData.flatMap(_.reportingMethod)
  ...
  private def getSuccessCall(isAgent: Boolean, incomeSourceType: IncomeSourceType): Call = {
    (isAgent, incomeSourceType) match {
      case (false, SelfEmployment) => routes.ManageObligationsController.show(isAgent, incomeSourceType)
      case (_, SelfEmployment) => routes.ManageObligationsController.show(isAgent, incomeSourceType)
      case (false, UkProperty) => routes.ManageObligationsController.show(isAgent, incomeSourceType)
      case (_, UkProperty) => routes.ManageObligationsController.show(isAgent, incomeSourceType)
      case (false, _) => routes.ManageObligationsController.show(isAgent, incomeSourceType)
      case (_, _) => routes.ManageObligationsController.show(isAgent, incomeSourceType)
    }
  }
