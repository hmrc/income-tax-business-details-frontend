...
class IncomeSourceAddedBackErrorController @Inject()(val authActions: AuthActions,
  ...
  def handleRequest(isAgent: Boolean, incomeSourceType: IncomeSourceType)
    ...
    val cannotGoBackRedirectUrl = if (isAgent) routes.ReportingMethodSetBackErrorController.showAgent(incomeSourceType)
    else routes.ReportingMethodSetBackErrorController.show(incomeSourceType)
    if (data.addIncomeSourceData.exists(addData => addData.incomeSourceCreatedJourneyComplete.contains(true))) {
      ...
      val postAction = if (isAgent) routes.IncomeSourceAddedBackErrorController.submitAgent(incomeSourceType)
      else routes.IncomeSourceAddedBackErrorController.submit(incomeSourceType)
