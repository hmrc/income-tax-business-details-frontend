  ...
  private def handleRequest(isAgent: Boolean, incomeSourceType: IncomeSourceType, isTriggeredMigration: Boolean)(implicit user: MtdItUser[_]): Future[Result] = {
    ...
    val linkUrl = {
      (isAgent, isTriggeredMigration) match {
        case (false, false)  => manageBusinessesRoutes.ManageYourBusinessesController.show().url
        case (true, false)   => manageBusinessesRoutes.ManageYourBusinessesController.show().url
        case (isAgent, true) => triggeredMigrationRoutes.CheckHmrcRecordsController.show(isAgent).url
      }
    }
