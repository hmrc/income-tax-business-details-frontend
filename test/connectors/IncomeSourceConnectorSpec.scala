...
import models.audit.{ExtendedAuditModel, IncomeSourceDetailsResponseAuditModel}
...
import testConstants.IncomeSourceDetailsTestConstants.singleBusinessAndPropertyMigrat2019
import uk.gov.hmrc.http.{HeaderCarrier, HttpReads, HttpResponse, SessionId}
import uk.gov.hmrc.play.bootstrap.config.ServicesConfig

import scala.concurrent.{ExecutionContext, Future}

class IncomeSourceConnectorSpec extends BaseConnectorSpec {
  ...
  val incomeSourceOverride: Seq[String] =
    Seq(
  ...
  trait Setup {
    ...
    val connector = new IncomeSourceConnector(mockHttpClientV2, mockAuditingService, getAppConfig)
