...
import testConstants.IncomeSourceDetailsTestConstants.{businessesAndPropertyIncome, emptyUIJourneySessionData, foreignPropertyIncomeWithCeasedForiegnPropertyIncome, ukPropertyIncomeWithCeasedUkPropertyIncome}
import testConstants.IncomeSourcesObligationsTestConstants.quarterlyObligationDatesSimple
import controllers.manageBusinesses.routes as manageBusinessRoutes
    ...
    s"submit" when {
      ...
      s"the user is authenticated as a $mtdRole" should {
        "redirect to ManageIncomeSources controller" in {
          ...
          val result = action(fakeRequest)
          if (isAgent)
            redirectLocation(result) shouldBe Some(manageBusinessRoutes.ManageYourBusinessesController.showAgent().url)
          else
            redirectLocation(result) shouldBe Some(manageBusinessRoutes.ManageYourBusinessesController.show().url)
        }
      }
      testMTDAuthFailuresForRole(action, mtdRole)(fakeRequest)
    }
