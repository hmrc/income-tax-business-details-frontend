...
import controllers.manageBusinesses.manage.routes as manageYourBusinessRoutes
...
import testConstants.IncomeSourceDetailsTestConstants.{completedUIJourneySessionData, emptyUIJourneySessionData, notCompletedUIJourneySessionData, ukPlusForeignPropertyAndSoleTraderWithLatency}
        ...
        s"the user is authenticated as s$testMtdRole" should {
          s"return ${Status.SEE_OTHER} and redirect to the Manage Obligations page for a property" when {
            "the user's property reporting method is updated to annual" in new SetupPOST {
              ...
              val expectedEndpoint: String = manageYourBusinessRoutes.ManageObligationsController.show(
