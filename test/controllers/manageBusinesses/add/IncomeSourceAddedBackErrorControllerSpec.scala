      ...
      s"submit${if (mtdRole != MTDIndividual) "Agent" else ""}(incomeSourceType = $incomeSourceType)" when {
        ...
        s"the user is authenticated as a $mtdRole" should {
          s"return ${Status.SEE_OTHER} and redirect to $incomeSourceType reporting method page" in {
            ...
            redirectLocation(result) shouldBe Some(addBusinessRoutes.IncomeSourceReportingFrequencyController.show(mtdRole != MTDIndividual, false, incomeSourceType).url)
          }
        }
        testMTDAuthFailuresForRole(action, mtdRole)(fakeRequest)
      }
