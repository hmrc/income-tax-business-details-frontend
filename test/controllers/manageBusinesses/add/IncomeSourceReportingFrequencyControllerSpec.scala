...
import testConstants.IncomeSourceDetailsTestConstants.notCompletedUIJourneySessionData

import java.time.LocalDate
import scala.concurrent.Future
import controllers.manageBusinesses.add.routes as addBusinessRoutes
import mocks.services.MockIncomeSourceRFService
        ...
        s"the user is authenticated as a $mtdRole" should {
          "return 303 SEE_OTHER and redirect to the check details page" when {
            s"changing answer, yes selected, previous choice was true and RF data exists" in {
              ...
              val expectedRedirectUrl =
                addBusinessRoutes.IncomeSourceRFCheckDetailsController.show(isAgent, incomeSourceType).url
          ...
          "return 303 SEE_OTHER and redirect to the ChooseTaxYear Page" when {
            s"changing answer, yes selected, previous choice was false (or missing) even if RF data exists" in {
              ...
              val expectedRedirectUrl =
                addBusinessRoutes.ChooseTaxYearController.show(isAgent, false, incomeSourceType).url
          ...
          "return 303 SEE_OTHER and redirect to the ChooseTaxYear Page" when {
            s"completing the form with yes selected and updates send successfully" in {
              ...
              val expectedRedirectUrl = addBusinessRoutes.ChooseTaxYearController.show(isAgent, false, incomeSourceType).url
          ...
          "return 303 SEE_OTHER and redirect to the check details page Page" when {
            s"completing the form with no selected and updates send successfully" in {
              ...
              val expectedRedirectUrl = addBusinessRoutes.IncomeSourceRFCheckDetailsController.show(isAgent, incomeSourceType).url
