...
import controllers.manageBusinesses.cease.routes as ceaseBusinessRoutes
...
class IncomeSourceEndDateControllerSpec extends MockAuthActions with MockSessionService with MockDateService{
  ...
  def getActions(isAgent: Boolean, incomeSourceType: IncomeSourceType, id: Option[String], mode: Mode): (Call, Call, Call) = {
    val manageBusinessesCall = if (isAgent) {
      manageBusinessRoutes.ManageYourBusinessesController.showAgent()
    } else {
          ...
          s"the user is authenticated as a $mtdRole" should {
            "redirect to CheckIncomeSourceDetails" when {
              "form is completed successfully" in {
                ...
                val redirectLocationResult = if (isAgent) ceaseBusinessRoutes.CeaseCheckIncomeSourceDetailsController.showAgent(incomeSourceType).url
                else ceaseBusinessRoutes.CeaseCheckIncomeSourceDetailsController.show(incomeSourceType).url
