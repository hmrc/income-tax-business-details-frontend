...
import testConstants.IncomeSourceDetailsTestConstants.{completedUIJourneySessionData, emptyUIJourneySessionData, ukPlusForeignPropertyAndSoleTraderPlusCeasedBusinessIncome}
        ...
        s"the user is authenticated as a $mtdRole" should {
          "redirect to end date controller" when {
            "cease declaration is completed" in {
              ...
              val optId = if (incomeSourceType == SelfEmployment) Some("test-id") else None
              redirectLocation(result) shouldBe Some(ceaseBusinessRoutes.IncomeSourceEndDateController.show(optId, incomeSourceType, isAgent, NormalMode).url)
