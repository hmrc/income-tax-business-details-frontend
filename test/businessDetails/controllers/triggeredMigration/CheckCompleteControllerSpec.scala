...
class CheckCompleteControllerSpec extends MockAuthActions {
  ...
  mtdAllRoles.foreach { mtdRole =>
    ...
    @unused val homeUrl = mtdRole match {
      case MTDIndividual => businessDetails.controllers.manageBusinesses.routes.ManageYourBusinessesController.show().url
      case _             => businessDetails.controllers.manageBusinesses.routes.ManageYourBusinessesController.showAgent().url
    }
    ...
    s"show(isAgent = $isAgent)" when {
      ...
      s"the user is authenticated as a $mtdRole" should {
        ...
        "redirect to the home page when the TriggeredMigration FS is disabled" in {
          ...
          redirectLocation(result).get should include("/manage-self-assessment/businesses")
        }
      }
