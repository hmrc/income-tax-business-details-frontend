...
import common.models.incomeSourceDetails.AddIncomeSourceData.{businessNameField, businessTradeField}
            ...
            "return 303 and redirect to add business start date" when {
              "the business name entered is valid" in {
                ...
                redirectLocation(result) mustBe Some(
                  addBusinessRoutes.AddIncomeSourceStartDateController.show(mtdRole != MTDIndividual, NormalMode, SelfEmployment).url
                )
              }
            }
          }
