...
import testConstants.IncomeSourceDetailsTestConstants.{businessIncome, notCompletedUIJourneySessionData}
...
import common.mocks.services.MockNextUpdatesService
...
class IncomeSourceAddedControllerSpec extends MockAuthActions with MockNextUpdatesService with MockSessionService with MockITSAStatusService {
  ...
  ".getNextUpdatesUrl" should {
    ...
    "return the correct agent url" in {
      testIncomeSourceAddedController.getNextUpdatesUrl(true) shouldBe s"${frontendAppConfig.homePageUrl(true)}/submission-deadlines"
    }

    "return the correct non-agent url" in {
      testIncomeSourceAddedController.getNextUpdatesUrl(false) shouldBe s"${frontendAppConfig.homePageUrl(false)}/submission-deadlines"
    }
  }
  ...
  ".getReportingFrequencyUrl" should {
    ...
    "return the correct agent url" in {
      testIncomeSourceAddedController.getReportingFrequencyUrl(true) shouldBe s"${frontendAppConfig.homePageUrl(true)}/reporting-frequency"
    }

    "return the correct non-agent url" in {
      testIncomeSourceAddedController.getReportingFrequencyUrl(false) shouldBe s"${frontendAppConfig.homePageUrl(false)}/reporting-frequency"
    }
  }
  ...
  ".getManageBusinessUrl" should {
    ...
    "return the correct agent url" in {
      testIncomeSourceAddedController.getManageBusinessUrl(true) shouldBe "/manage-self-assessment/businesses/agents/manage-your-businesses"
    }

    "return the correct non-agent url" in {
      testIncomeSourceAddedController.getManageBusinessUrl(false) shouldBe "/manage-self-assessment/businesses/manage-your-businesses"
    }
  }
      ...
      s"the user is authenticated as a MTDIndividual - $incomeSourceType" should {
        ...
        "redirect to the cannot-go-back page" when {
         "there is no journey session" in {
           ...
           redirectLocation(result) shouldBe Some("/manage-self-assessment/businesses/manage-your-businesses/cannot-go-back")
         }
        }
      ...
      s"the user is authenticated as a MTDPrimaryAgent - $incomeSourceType" should {
        ...
        "redirect to the cannot go back page" when {
          "there is no journey session" in {
            ...
            status(result) shouldBe SEE_OTHER
            redirectLocation(result) shouldBe Some("/manage-self-assessment/businesses/agents/manage-your-businesses/cannot-go-back")
