...
import common.models.incomeSourceDetails.{AddIncomeSourceData, Address, BusinessAddressModel, Country}
...
class AddBusinessAddressControllerSpec extends MockAuthActions
  ...
  val postAction: Call = addBusinessRoutes.AddBusinessAddressController.submit(None, mode = NormalMode)
  val postActionCheck: Call = addBusinessRoutes.AddBusinessAddressController.submit(None, mode = CheckMode)
          ...
          "redirect to the business check answers page" when {
            "valid data received" in {
              val checkAnswersUrl = if (mtdRole == MTDIndividual) {
                "/manage-self-assessment/businesses/manage-your-businesses/add-sole-trader/business-check-answers"
              } else {
                "/manage-self-assessment/businesses/agents/manage-your-businesses/add-sole-trader/business-check-answers"
              }
              setupMockSuccess(mtdRole)
              mockItsaStatusRetrievalAction(businessesAndPropertyIncome)
              setupMockGetIncomeSourceDetails(businessesAndPropertyIncome)
