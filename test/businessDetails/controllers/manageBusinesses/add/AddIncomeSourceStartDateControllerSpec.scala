...
import common.models.incomeSourceDetails.AddIncomeSourceData.dateStartedField
...
import testConstants.IncomeSourceDetailsTestConstants.{noIncomeDetails, notCompletedUIJourneySessionData}
...
class AddIncomeSourceStartDateControllerSpec extends MockAuthActions
  ...
  def getBackUrl(isAgent: Boolean, mode: Mode, incomeSourceType: IncomeSourceType): String = ((isAgent, mode, incomeSourceType) match {
    ...
    case (false, NormalMode, _) => addBusinessRoutes.AddPropertyController.show(isAgent)
    case (_, NormalMode, _) => addBusinessRoutes.AddPropertyController.show(isAgent)
          ...
          s"the user is authenticated as a $mtdRole" should {
            ...
            s"return ${Status.SEE_OTHER}: redirect to the relevant You Cannot Go Back page" when {
              s"user has already completed the journey (${incomeSourceType.key})" in {
                ...
                val redirectUrl = if (isAgent) addBusinessRoutes.ReportingMethodSetBackErrorController.showAgent(incomeSourceType).url
              ...
              s"user has already added their income source (${incomeSourceType.key})" in {
                ...
                val result = action(fakeRequest)
                status(result) shouldBe SEE_OTHER
                val redirectUrl = if (isAgent) addBusinessRoutes.IncomeSourceAddedBackErrorController.showAgent(incomeSourceType).url
