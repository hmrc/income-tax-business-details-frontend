...
class CheckCompleteViewSpec extends TestSupport {
  ...
  def nextUpdatesLink(isAgent: Boolean): String =
    if(isAgent) "/manage-self-assessment/businesses/agents/submission-deadlines"
    else "/manage-self-assessment/businesses/submission-deadlines"
  ...
  class Setup(isAgent: Boolean) {
    val homeCall: Call = if(isAgent) businessDetails.controllers.manageBusinesses.routes.ManageYourBusinessesController.showAgent() else businessDetails.controllers.manageBusinesses.routes.ManageYourBusinessesController.show()
