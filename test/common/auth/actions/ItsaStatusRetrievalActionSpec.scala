  ...
  ".refine()" when {
    ...
    "CY+1YouMustWaitToSignUpPageEnabled feature switch is Enabled" should {
      ...
      "redirect to YouMustWaitToSignUp page when only next-year ITSA status exists - Agent" in {
        ...
        result.left.value.header.headers("LOCATION") shouldBe appConfig.youMustWaitToSignUpUrl(true)
      }
      ...
      "redirect to YouMustWaitToSignUp page when only next-year ITSA status exists - Individual" in {
        ...
        result.left.value.header.status shouldBe SEE_OTHER
        result.left.value.header.headers("LOCATION") shouldBe appConfig.youMustWaitToSignUpUrl(false)
      }
      ...
      "return Individual error page" in {
        ...
        result match {
          case Left(res) =>
            ...
            feedbackUrl shouldBe "/manage-self-assessment/businesses/feedback" // to help distinguish error pages checking for feedback url since page titles are the same
          case Right(_) =>
      ...
      "return Agent error page" in {
        ...
        result match {
          case Left(res) =>
            ...
            res.header.status shouldBe INTERNAL_SERVER_ERROR
            title shouldBe "Sorry, there is a problem with the service - GOV.UK"
            agentFeedbackUrl shouldBe "/manage-self-assessment/businesses/agents/feedback" // to help distinguish error pages checking for feedback url since page titles are the same
          case Right(_) =>
