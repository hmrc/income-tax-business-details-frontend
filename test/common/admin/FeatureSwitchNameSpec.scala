...
import common.models.admin.{FeatureSwitchName, DisplayBusinessStartDate, NotRequiredFS, TriggeredMigration}
...
class FeatureSwitchNameSpec extends PlaySpec{
  ...
  "FeatureSwitchName.reads" should {
    "deserialize a valid FeatureSwitchName from JSON string" in {
      val json = JsString(DisplayBusinessStartDate.name)
      json.as[FeatureSwitchName] mustBe DisplayBusinessStartDate
    }

    "deserialize another valid FeatureSwitchName" in {
      val json = JsString(TriggeredMigration.name)
      json.as[FeatureSwitchName] mustBe TriggeredMigration
    }
