...
trait MockIncomeSourceConnector extends UnitSpec with BeforeAndAfterEach {

  lazy val mockIncomeSourceConnector: IncomeSourceConnector = mock(classOf[IncomeSourceConnector])
  ...
  def verifyMockIncomeSourceDetailsResponse(noOfCalls: Int): Future[IncomeSourceDetailsResponse] =
    verify(mockIncomeSourceConnector, times(noOfCalls)).getIncomeSources()(ArgumentMatchers.any(), ArgumentMatchers.any())
