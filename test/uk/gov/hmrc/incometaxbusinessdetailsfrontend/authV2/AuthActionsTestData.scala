...
package uk.gov.hmrc.incometaxbusinessdetailsfrontend.authV2

import uk.gov.hmrc.incometaxbusinessdetailsfrontend.controllers.agent.AuthUtils.*
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.admin.FeatureSwitch
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.incomeSourceDetails.{IncomeSourceDetailsError, IncomeSourceDetailsModel}
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.sessionData.SessionCookieData
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.sessionData.SessionDataGetResponse.{SessionDataGetSuccess, SessionDataNotFound}
...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.auth.authV2.models.{AgentClientDetails, AuthUserDetails, AuthorisedAgentWithClientDetailsRequest, AuthorisedAndEnrolledRequest, AuthorisedUserRequest}
...
object AuthActionsTestData {
  ...
  def primaryAgentPredicate(mtdItId: String = testMtditid): Predicate =
    Enrolment(mtdEnrolmentName)
      .withIdentifier(agentIdentifier, mtdItId)
      .withDelegatedAuthRule(primaryAgentAuthRule)

  def secondaryAgentPredicate(mtdItId: String = testMtditid): Predicate =
  ...
  def delegatedEnrolmentPredicate(mtdItId: String,
                                  isSupportingAgent: Boolean): Predicate =
  ...
  def getAllEnrolmentsIndividual(hasNino: Boolean, hasSA: Boolean): Enrolments = {
    var enrolmentList = List(mtdEnrolment)
    if (hasNino) enrolmentList :+= ninoEnrolment
    if (hasSA) enrolmentList :+= saEnrolment
    Enrolments(enrolmentList.toSet)
  }

  def getAllEnrolmentsAgent(hasNino: Boolean, hasSA: Boolean): Enrolments = {
  ...
  def getMtdItUser(
    ...
    val (mtdUserRole, enrolments, optClientDetails) =
      affinityGroup match {
        case Agent if isSupportingAgent =>
          (MTDSupportingAgent, getAllEnrolmentsAgent(true, true), Some(getAgentClientDetails(true)))
        case Agent =>
          (MTDPrimaryAgent, getAllEnrolmentsAgent(true, true), Some(getAgentClientDetails(true)))
        case _ =>
          (MTDIndividual, getAllEnrolmentsIndividual(true, true), None)
      }
