...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.admin.{FeatureSwitchName, TriggeredMigration}
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.incomeSourceDetails.{BusinessDetailsModel, TaxYear}
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.itsaStatus.ITSAStatus.*
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.itsaStatus.{ITSAStatusResponseModel, StatusDetail, StatusReason}
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.liabilitycalculation.*
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.models.triggeredMigration.TriggeredMigrationSessionData
...
import uk.gov.hmrc.incometaxbusinessdetailsfrontend.auth.authV2.actions.TriggeredMigrationRetrievalAction
    ...
    "redirect to the home page" when {
      ...
      "the user is unconfirmed and their CY ITSA status is missing but their CY+1 ITSA status is available" in {
        ...
        when(mockItsaStatusService.getITSAStatusDetail(eqTo(mockDateServiceInterface.getCurrentTaxYear), any(), any())(any(), any(), any()))
          .thenReturn(Future(List(ITSAStatusResponseModel("2026-27", None))))
