...
class AgentItvcErrorHandlerSpec extends UnitSpec with ScalaFutures {
  ...
  "AgentItvcErrorHandler" should {
    "render standard error template as html" in {
      val handler = app.injector.instanceOf[AgentItvcErrorHandler]
    ...
    "return unauthorized html page for supporting agents" in {
      val handler = app.injector.instanceOf[AgentItvcErrorHandler]
      val result = handler.supportingAgentUnauthorised()(request)
      val futureResult = Future.successful(result)

      status(futureResult) shouldBe UNAUTHORIZED
      contentType(futureResult) shouldBe Some("text/html")
      contentAsString(futureResult) should include("You are not authorised to access this page")
    }
  }
}
